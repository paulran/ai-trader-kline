...
实时模式采用**时间对齐机制**确保获取到的K线数据是完整的：

- **1分钟K线**：在每分钟的第10秒获取数据（如 10:00:10, 10:01:10, 10:02:10...）
- **15分钟K线**：在每15分钟的第10秒获取数据（如 10:00:10, 10:15:10, 10:30:10...）

**为什么要延后10秒？**
- OKX的K线数据在整点时刻可能还在生成中，延后10秒可以确保获取到完整的K线数据

**数据过滤**
- 获取到的K线数据会自动去除最后一条（可能不完整的数据）
- 只使用已完成的有效K线数据进行分析

**禁用时间对齐**
```bash
python main.py --mode realtime --bar 1m --no_align
```
...
| 参数 | 说明 | 默认值 |
|-----|------|-------|
| `--bar` | K线周期: `1m` (1分钟) 或 `15m` (15分钟) | `1m` |
| `--interval` | 刷新间隔（秒） | 1m=60秒, 15m=900秒 |
| `--model_path` | 强化学习模型路径 | 自动检测 |
| `--no_llm` | 禁用LLM分析，仅使用规则分析 | 不启用 |
| `--no_save` | 不保存K线数据到CSV文件 | 不启用 |
| `--simulate` | 启用模拟交易 | 不启用 |
