...
def main():
    parser = argparse.ArgumentParser(description='实时K线分析器 - 从OKX定时获取K线数据并进行AI分析')
    
    parser.add_argument('--bar', type=str, default='1m',
                        choices=['1m', '15m'],
                        help='K线周期: 1m (1分钟) 或 15m (15分钟) (默认: 1m)')
    
    parser.add_argument('--interval', type=int, default=None,
                        help='刷新间隔（秒），默认: 1m=60秒, 15m=900秒')
    
    parser.add_argument('--model_path', type=str, default=None,
                        help='强化学习模型路径（可选）')
    
    parser.add_argument('--no_llm', action='store_true', default=False,
                        help='禁用LLM分析，仅使用规则分析和强化学习')
